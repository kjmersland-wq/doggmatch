...
const kindLabels: Record<WeekKind, { en: string; no: string; pl: string; dk: string; se: string; fi: string; de: string; fr: string; nl: string }> = {
  ...
  training: { en: "Training", no: "Trening", pl: "Trening", dk: "Træning", se: "Träning", fi: "Harjoittelu", de: "Training", fr: "Entraînement", nl: "Training" },
  food: { en: "Food", no: "Mat", pl: "Jedzenie", dk: "Mad", se: "Mat", fi: "Ruoka", de: "Futter", fr: "Nourriture", nl: "Voeding" },
...
function walkLine(dog: DogProfile | undefined, care: CareProfile): { label: string; detail: string } {
  ...
  if (dog?.ageStage === "puppy") {
    return pick({
      ...
      de: { label: "Zwei kurze Spaziergänge", detail: "Kurz und schnüffelreich – kleine Beine werden schnell müde" },
      fr: { label: "Deux courtes promenades", detail: "Courtes et pleines de reniflages — les petites pattes se fatiguent vite" },
      nl: { label: "Twee korte wandelingen", detail: "Kort en vol snuffelen — kleine pootjes worden snel moe" },
    });
  }
  if (dog?.ageStage === "senior") {
    return pick({
      ...
      de: { label: "Ein ruhiger Spaziergang", detail: "Im Tempo des Hundes, nicht in Ihrem" },
      fr: { label: "Une promenade tranquille", detail: "Au rythme du chien, pas au vôtre" },
      nl: { label: "Een rustige wandeling", detail: "In het tempo van de hond, niet het jouwe" },
    });
  }
  if (energy >= 4 && activity !== "gentle") {
    return pick({
      ...
      de: { label: "Ein langer Spaziergang", detail: "Dieser Hund braucht eine ordentliche Bewegung" },
      fr: { label: "Une longue promenade", detail: "Ce chien a besoin de bien se dégourdir les pattes" },
      nl: { label: "Een lange wandeling", detail: "Deze hond heeft behoefte aan een flinke beenstrekker" },
    });
  }
  if (energy <= 2 || activity === "gentle") {
    return pick({
      ...
      de: { label: "Ein entspannter Spaziergang", detail: "Ruhig, mit Zeit zum Schnüffeln" },
      fr: { label: "Une promenade facile", detail: "Tranquille, avec le temps de renifler" },
      nl: { label: "Een makkelijke wandeling", detail: "Rustig tempo, met tijd om te snuffelen" },
    });
  }
  return pick({
    ...
    fr: { label: "Une bonne promenade", detail: "Environ une demi-heure, avec du temps pour renifler" },
...
export function buildWeek(
  ...
  const days: WeekDay[] = getDayNames().map((name, index) => {
    ...
    items.push({
      ...
      detail: pick({ en: "Weighed rather than guessed", no: "Veid, ikke gjettet", pl: "Zważone, a nie odmierzone na oko", dk: "Vejet, ikke gættet", se: "Vägt, inte gissat", fi: "Punnittu, ei arvattu", de: "Abgewogen statt geschätzt", fr: "Pesé plutôt que deviné", nl: "Afgewogen in plaats van geschat" }),
    });
