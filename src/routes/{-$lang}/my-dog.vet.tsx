...
const copy = {
  ...
  no: {
    ...
    noVisits:
      "Ingenting lagret ennå. Når du har vært der, ta vare på notatene her — det er nyttig å se tilbake på.",
    remove: "Fjern",
    vetNote:
  ...
  pl: {
    ...
    noVisits:
      "Nic jeszcze nie zapisano. Po wizycie zachowaj tu notatki — dobrze mieć do czego wrócić.",
    remove: "Usuń",
    vetNote:
  ...
  dk: {
    ...
    noVisits:
      "Intet gemt endnu. Når du har været der, kan du gemme noterne her — det er nyttigt at kunne se tilbage på.",
    remove: "Fjern",
    vetNote:
  ...
  se: {
    ...
    noVisits:
      "Inget sparat än. När du har varit där, spara anteckningarna här — det är bra att kunna gå tillbaka till.",
    remove: "Ta bort",
    vetNote:
    ...
    noVisits:
      "Ei vielä tallennettuja käyntejä. Kun olet käynyt, säilytä muistiinpanot täällä — niihin on hyvä palata.",
    remove: "Poista",
    vetNote:
  ...
  de: {
    ...
    titleFor: (name: string) => `${name} zum Tierarzt bringen`,
    titleFallback: "Ihren Hund zum Tierarzt bringen",
    intro:
      "Es ist leicht, die Hälfte dessen zu vergessen, was man sagen wollte, sobald man erst mal im Sprechzimmer sitzt. Schreiben Sie es hier auf, drucken Sie es aus, und nehmen Sie es mit.",
    beforeWeGo: "Bevor wir losfahren",
    date: "Datum",
    weight: "Gewicht (kg)",
    reason: "Warum wir hingehen",
    ...
    printIt: "Drucken",
    addDogNote: "Fügen Sie zuerst Ihren Hund hinzu, wenn Sie dies für das nächste Mal aufbewahren möchten.",
    visitsSavedTitle: "Gespeicherte Besuche",
    noVisits: "Noch nichts gespeichert. Sobald Sie dort waren, bewahren Sie die Notizen hier auf — es hilft, zurückzuschauen.",
    remove: "Entfernen",
    vetNote:
      "Diese Angaben tragen Sie selbst ein, um Ihre Beobachtungen und Fragen für einen Tierarztbesuch zu ordnen. Ihr Tierarzt ist derjenige, der Ihren Hund untersuchen und Sie beraten kann.",
  },
  fr: {
    eyebrow: "Visites vétérinaires",
    titleFor: (name: string) => `Emmener ${name} chez le vétérinaire`,
    titleFallback: "Emmener votre chien chez le vétérinaire",
    intro:
      "Il est facile d'oublier la moitié de ce que vous vouliez dire une fois dans la pièce. Notez-le ici, imprimez-le et emportez-le avec vous.",
    ...
    printIt: "Imprimer",
    addDogNote: "Ajoutez d'abord votre chien si vous souhaitez conserver ceci pour la prochaine fois.",
    visitsSavedTitle: "Visites enregistrées",
    noVisits: "Rien d'enregistré pour le moment. Une fois la visite passée, conservez les notes ici — c'est utile pour s'y référer.",
    remove: "Supprimer",
    vetNote:
      "Ces informations sont saisies par vous pour organiser vos observations et vos questions en vue d'une visite vétérinaire. Votre vétérinaire est la personne qui pourra examiner votre chien et vous conseiller.",
  },
  nl: {
    eyebrow: "Dierenartsbezoeken",
    titleFor: (name: string) => `${name} naar de dierenarts brengen`,
    titleFallback: "Uw hond naar de dierenarts brengen",
    intro:
      "Het is gemakkelijk om de helft te vergeten van wat u van plan was te zeggen zodra u in de spreekkamer zit. Schrijf het hier op, druk het af en neem het mee.",
    beforeWeGo: "Voordat we gaan",
    date: "Datum",
    weight: "Gewicht (kg)",
    reason: "Waarom we gaan",
    questions: "Wat ik wil vragen",
    notes: "Wat ons is verteld",
    followUp: "Vervolgafspraak",
    next: "Volgende afspraak",
    saveVisit: "Dit bezoek opslaan",
    printIt: "Afdrukken",
    addDogNote: "Voeg eerst uw hond toe als u dit wilt bewaren voor de volgende keer.",
    visitsSavedTitle: "Opgeslagen bezoeken",
    noVisits: "Nog niets opgeslagen. Zodra u er bent geweest, bewaar de notities hier — het is fijn om erop terug te kunnen kijken.",
    remove: "Verwijderen",
    vetNote:
      "Deze informatie vult u zelf in om uw observaties en vragen voor een dierenartsbezoek te ordenen. Uw dierenarts is degene die uw hond kan onderzoeken en u kan adviseren.",
  },
} as const;
        ...
        <div className="grid gap-6 lg:grid-cols-[1.3fr_1fr]">
          <Panel title={c.beforeWeGo}>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              <ButtonLink
                to={withLangPrefix("/my-dog/pack")}
                search={{ docs: "vet" } as never}
                tone="outline"
                size="lg"
              >
                {c.printIt}
              ...
              {visits.length === 0 ? (
                ...
                <ul className="grid gap-3">
                  {visits.map((v) => (
                    <li
                      key={v.id}
                      className="rounded-[1rem] border border-border bg-surface px-4 py-3"
                    >
